...
$(document).ready(function() {
	...
	var jsbeginAddr = "<?php echo $beginAddr ?>".split(".");
	var jsendAddr = "<?php echo $endAddr ?>".split(".");
	...
	//LNT_EMU
	jsbeginAddr[0] = parseInt(jsbeginAddr[0]);
	jsbeginAddr[1] = parseInt(jsbeginAddr[1]);
	jsbeginAddr[2] = parseInt(jsbeginAddr[2]);
	jsbeginAddr[3] = parseInt(jsbeginAddr[3]);

	//LNT_EMU

	jsendAddr[0] = parseInt(jsendAddr[0]);
	jsendAddr[1] = parseInt(jsendAddr[1]);
	jsendAddr[2] = parseInt(jsendAddr[2]);
	jsendAddr[3] = parseInt(jsendAddr[3]);
	...
	if (isEnabledDMZ) {
		...
		if(isValid && !IsBlank("dmz_host_address_")) {
			if (host == jsGatewayIP){
				...
				if((jsGwIP[0] != host0) || (jsGwIP[1] != host1) || (jsGwIP[2] != host2) || host3<jsbeginAddr[3] || host3>jsendAddr[3]){
					jAlert('DMZ v4 Host IP is not in valid range:\n' + jsGwIP[0] + '.' + jsGwIP[1] + '.' + jsGwIP[2] + '.' + '[' + jsbeginAddr[3] + '~' + jsendAddr[3] + ']');
					isValid = false;
				}
			}
